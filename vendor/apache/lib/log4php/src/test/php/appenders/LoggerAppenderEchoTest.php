...
class LoggerAppenderEchoTest extends PHPUnit_Framework_TestCase {
	...
	private $config2 = array(
			...
			'appenders' => array(
					'default' => array('class' => 'LoggerAppenderEcho',
							'layout' => array('class' => 'LoggerLayoutSimple'),
							'params' => array('htmlLineBreaks' => true))));
	...
	private $config3 = array(
			...
			'appenders' => array(
					'default' => array('class' => 'LoggerAppenderEcho',
							'layout' => array('class' => 'LoggerLayoutSimple'),
							'params' => array('htmlLineBreaks' => 'foo'))));
	...
	public function testAppend() {
		...
		$actual = ob_get_clean();
		$expected = "INFO - This is a test" . PHP_EOL . "DEBUG - And this too"
				. PHP_EOL;
	...
	public function testHtmlLineBreaks() {
		...
		$log->debug("And this too");
		$actual = ob_get_clean();
		$expected = "INFO - This is a test<br />" . PHP_EOL
				. "With more than one line<br />" . PHP_EOL
				. "DEBUG - And this too<br />" . PHP_EOL;
	...
	public function testEchoHtml() {
		...
		$expected = "ERROR - This message<br />\nis in several lines<br />\r\nto test various line breaks.<br />"
				. PHP_EOL;

		$event = new LoggerLoggingEvent("LoggerAppenderEchoTest",
				new Logger("TEST"), LoggerLevel::getLevelError(), $msg);
		ob_start();
		$appender->append($event);
		$actual = ob_get_clean();
		self::assertEquals($expected, $actual);
