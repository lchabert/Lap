...
class LoggerAppenderRollingFileTest extends PHPUnit_Framework_TestCase {
	...
	public function testLoggingViaLogger() {
		...
		$this
				->assertFalse(
						file_exists(PHPUNIT_TEMP_DIR . '/TEST-rolling.txt.3'),
						'should not roll over three times');
	}
	...
	public function testRolloverWithCompression() {
		...
		$firstCompressedRollingFile = PHPUNIT_TEMP_DIR
				. '/TEST-rolling.txt.1.gz';
		$this
				->assertTrue(file_exists($firstCompressedRollingFile),
						'TEST-rolling.txt.1.gz not found');

		$firstUncompressedRollingField = PHPUNIT_TEMP_DIR
				. '/TEST-rolling.txt.1';
		$this
				->assertFalse(file_exists($firstUncompressedRollingField),
						'TEST-rolling.txt.1 should be replaced by compressed');

		$secondCompressedRollingFile = PHPUNIT_TEMP_DIR
				. '/TEST-rolling.txt.2.gz';
		$this
				->assertTrue(file_exists($secondCompressedRollingFile),
						'TEST-rolling.txt.2.gz not found');

		$secondUncompressedRollingField = PHPUNIT_TEMP_DIR
				. '/TEST-rolling.txt.2';
		$this
				->assertFalse(file_exists($secondUncompressedRollingField),
						'TEST-rolling.txt.2 should be replaced by compressed');
