...
class LoggerAppenderMongoDBTest extends PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		if (!extension_loaded('mongo')) {
			$this
					->markTestSkipped('The Mongo extension is not available.');
		} else {
			$this->appender = new LoggerAppenderMongoDB('mongo_appender');
			$this->event = LoggerTestHelper::getErrorEvent(
					'mongo logging event', 'test_mongo');
		}
	}
	...
	public function testActivateOptions() {
		...
		$this
				->assertInstanceOf('MongoCollection',
	...
	public function testFormat() {
		...
		$this
				->assertTrue(
						is_int($record['lineNumber'])
								|| $record['lineNumber'] == 'NA');
	}

	public function testFormatThrowableInfo() {
		$this->appender->activateOptions();
		$event = new LoggerLoggingEvent('testFqcn', new Logger('test.Logger'),
				LoggerLevel::getLevelWarn(), 'test message', microtime(true),
		...
		$this
				->assertContains(
						'[internal function]: LoggerAppenderMongoDBTest',
	...
	public function testFormatThrowableInfoWithInnerException() {
		...
		$event = new LoggerLoggingEvent('testFqcn', new Logger('test.Logger'),
				LoggerLevel::getLevelWarn(), 'test message', microtime(true),
				new Exception('test exception', 1,
						new Exception('test exception inner', 2)));
		...
		$this
				->assertEquals('test exception inner',
	...
	private function logOne($event) {
		...
		$this
				->assertNotNull($record,
						'Could not read the record from the database.');
		return $record;
	}
}
