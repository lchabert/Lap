...
class LoggerAppenderFirePHPTest extends PHPUnit_Framework_TestCase {
	...
	private $config = array(
			...
			'appenders' => array(
					'default' => array('class' => 'LoggerAppenderFirePHP',
	...
	public function setUp() {
		if (!method_exists('FirePHP', 'to')) {
			self::markTestSkipped(
					"Please install 'FirePHP' in order to run this test");
		}
	}

	private function createEvent($message, $level) {
		$eventMock = new LoggerLoggingEvent("LoggerAppenderFirePHPTest",
				new Logger("TEST"), LoggerLevel::toLevel($level), $message);
	...
	public function assertLog($console, $expectedMessage, $logLevel,
		...
		$this
				->assertEquals($message, $console->getMessage(),
						'log message is wrong');
		$this->assertEquals(1, $console->getCalls(), 'wasn\'t called once');
		$this
				->assertEquals($calledMethod, $console->getCalledMethod(),
