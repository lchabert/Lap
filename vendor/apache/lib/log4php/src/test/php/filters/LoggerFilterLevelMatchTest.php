...
class LoggerFilterLevelMatchTest extends PHPUnit_Framework_TestCase {
	...
	public function testDecideAccept() {
		...
		foreach ($levels as $level) {
			...
			foreach ($events as $event) {
				...
				$expected = ($event->getLevel() == $level) ? LoggerFilter::ACCEPT
	...
	public function testDecideDeny() {
		...
		foreach ($levels as $level) {
			...
			foreach ($events as $event) {
				...
				$expected = ($event->getLevel() == $level) ? LoggerFilter::DENY
						: LoggerFilter::NEUTRAL;
				$actual = $filter->decide($event);
	...
	public function testAcceptConfig() {
		...
		$config['appenders']['default']['filters'] = array(
				...
				array('class' => 'LoggerFilterLevelMatch',
						'params' => array('levelToMatch' => 'info',
								'acceptOnMatch' => true)),
	...
	public function testDenyConfig() {
		...
		$expected = "TRACE - Test" . PHP_EOL . "DEBUG - Test" . PHP_EOL
				. "WARN - Test" . PHP_EOL . "ERROR - Test" . PHP_EOL
				. "FATAL - Test" . PHP_EOL;
