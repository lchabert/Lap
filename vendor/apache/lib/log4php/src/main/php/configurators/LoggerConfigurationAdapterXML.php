...
class LoggerConfigurationAdapterXML implements LoggerConfigurationAdapter {
	...
	private $config = array('appenders' => array(), 'loggers' => array(),
			'renderers' => array(),);
	...
	private function loadXML($url) {
		...
		if ($xml === false) {
			...
			throw new LoggerException(
					"Error loading configuration file: " . trim($errorStr));
		}
	...
	private function parseAppender(SimpleXMLElement $node) {
		...
		if (isset($node['threshold'])) {
			$appender['threshold'] = $this
					->getAttributeValue($node, 'threshold');
		}
	...
	private function parseParameters($paramsNode) {
		...
		foreach ($paramsNode->param as $paramNode) {
			if (empty($paramNode['name'])) {
				$this
						->warn(
								"A <param> node is missing the required 'name' attribute. Skipping parameter.");
				continue;
			}
	...
	private function parseLogger(SimpleXMLElement $node) {
		...
		if (empty($name)) {
			$this
					->warn(
							"A <logger> node is missing the required 'name' attribute. Skipping logger definition.");
			return;
		}
		...
		if (isset($node['additivity'])) {
			$logger['additivity'] = $this
					->getAttributeValue($node, 'additivity');
		}
