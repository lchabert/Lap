...
class ContainerAwareProviderTest extends \PHPUnit_Framework_TestCase {
	public function testHas() {
		$provider = new ContainerAwareProvider(
				...
				'first', array('first' => 'first', 'second' => 'dummy'));
	...
	public function testGetExistentRenderer() {
		...
		$container = $this
				->getMock(
						'Symfony\Component\DependencyInjection\ContainerInterface');
		$container->expects($this->once())->method('get')->with('renderer')
				->will($this->returnValue($renderer));
		$provider = new ContainerAwareProvider($container, 'custom',
				array('default' => 'renderer', 'custom' => 'other'));
	...
	public function testGetDefaultRenderer() {
		...
		$container->expects($this->once())->method('get')->with('renderer')
				->will($this->returnValue($renderer));
		$provider = new ContainerAwareProvider($container, 'default',
				array('default' => 'renderer'));
