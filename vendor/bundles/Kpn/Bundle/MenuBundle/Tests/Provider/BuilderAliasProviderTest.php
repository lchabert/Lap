...
class BuilderAliasProviderTest extends \PHPUnit_Framework_TestCase {
	...
	public function testGetExistentMenu() {
		...
		$factory->expects($this->once())->method('createItem')
				->with('Main menu')->will($this->returnValue($item));

		$provider = new BuilderAliasProvider($this->createMockKernelForStub(),
				$this
						->getMock(
								'Symfony\Component\DependencyInjection\ContainerInterface'),
				$factory);
	...
	public function testGetContainerAwareMenu() {
		...
		$container = $this
				->getMock(
						'Symfony\Component\DependencyInjection\ContainerInterface');
		$container->expects($this->once())->method('get')->with('test');
	...
	public function testBundleInheritanceParent() {
		...
		$provider = new BuilderAliasProvider($this->createTestKernel(),
				$this
						->getMock(
								'Symfony\Component\DependencyInjection\ContainerInterface'),
				$factory);
	...
	public function testBundleInheritanceChild() {
		...
		$provider = new BuilderAliasProvider(
				$this
						->createTestKernel(
								'Knp\Bundle\MenuBundle\Tests\Stubs\Child'),
				$this
						->getMock(
								'Symfony\Component\DependencyInjection\ContainerInterface'),
				$factory);
	...
	private function createMockKernelForStub() {
		...
		$bundle->expects($this->once())->method('getNamespace')
	...
	private function createTestKernel($childNamespace = 'Bar',
			$parentNamespace = 'Knp\Bundle\MenuBundle\Tests\Stubs') {
		$bundleInterface = version_compare(Kernel::VERSION, '2.1-dev', '<') ? 'Knp\Bundle\MenuBundle\Tests\Stubs\ContainerAwareBundleInterface' // Symfony 2.0 misses the extend in the interface
 : 'Symfony\Component\HttpKernel\Bundle\BundleInterface';
		...
		$childBundle->expects($this->any())->method('getNamespace')
				->will($this->returnValue($childNamespace));
		$childBundle->expects($this->any())->method('getName')
				->will($this->returnValue('BarBundle'));
		$childBundle->expects($this->any())->method('getParent')
				->will($this->returnValue('FooBundle'));
