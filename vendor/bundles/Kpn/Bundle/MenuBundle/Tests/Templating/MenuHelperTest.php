...
class MenuHelperTest extends \PHPUnit_Framework_TestCase {
	public function testGet() {
		...
		$helperMock->expects($this->once())->method('get')
				->with($this->equalTo('test'),
						$this->equalTo(array('pathArray')))
		...
		$this
				->assertEquals('returned value',
						$helper->get('test', array('pathArray')));
	}
	...
	public function testGetMenuWithOptions() {
		...
		$this
				->assertSame($menu,
						$helper->get('default', array(), array('foo' => 'bar')));
	}

	public function testRender() {
		$helperMock = $this->getMockBuilder('Knp\Menu\Twig\Helper')
				->disableOriginalConstructor()->getMock();
		$helperMock->expects($this->once())->method('render')
		...
		$this
				->assertEquals('returned value',
						$helper->render('test', array('options')));
	}
