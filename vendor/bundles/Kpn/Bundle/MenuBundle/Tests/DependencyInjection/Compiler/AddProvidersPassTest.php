...
class AddProvidersPassTest extends \PHPUnit_Framework_TestCase {
	...
	public function testProcessForOneProvider() {
		...
		$containerBuilderMock->expects($this->once())
				...
				->will(
						$this
								->returnValue(
										array('id' => array('provider_tag1'))));
		$containerBuilderMock->expects($this->once())->method('setAlias')
				->with($this->equalTo('knp_menu.menu_provider'),
						$this->equalTo('id'));
	...
	public function testProcessForManyProviders() {
		...
		$definitionMock->expects($this->once())->method('replaceArgument')
		...
		$containerBuilderMock->expects($this->once())->method('getDefinition')
