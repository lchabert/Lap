...
class AddVotersPassTest extends \PHPUnit_Framework_TestCase {
	public function testProcessWithoutProviderDefinition() {
		...
		$containerBuilder->expects($this->once())->method('hasDefinition')
	...
	public function testProcessWithAlias() {
		...
		$definitionMock->expects($this->at(0))->method('addMethodCall')
				->with($this->equalTo('addVoter'),
						$this->equalTo(array(new Reference('id'))));
		$definitionMock->expects($this->at(1))->method('addMethodCall')
				->with($this->equalTo('addVoter'),
						$this->equalTo(array(new Reference('foo'))));

		$listenerMock = $this
				->getMockBuilder(
						'Symfony\Component\DependencyInjection\Definition')
				->disableOriginalConstructor()->getMock();
		$listenerMock->expects($this->once())->method('addMethodCall')
				->with($this->equalTo('addVoter'),
						$this->equalTo(array(new Reference('foo'))));

		$containerBuilderMock = $this
				->getMock(
						'Symfony\Component\DependencyInjection\ContainerBuilder');
		$containerBuilderMock->expects($this->once())->method('hasDefinition')
								...
								->returnValue(
										array(
												...
												'foo' => array(
														'tag1' => array(
																'request' => true)))));
		$containerBuilderMock->expects($this->at(1))->method('getDefinition')
				->with($this->equalTo('knp_menu.matcher'))
				->will($this->returnValue($definitionMock));
		$containerBuilderMock->expects($this->at(2))->method('getDefinition')
