...
class LdapAuthenticationProvider extends UserAuthenticationProvider {
		...
		try {
			...
			if (!$user instanceof LdapUserInterface) {
				throw new AuthenticationServiceException(
						'The user provider must return a LdapUserInterface object.');
			}
	...
	protected function checkAuthentication(UserInterface $user,
		...
		if ($currentUser instanceof LdapUserInterface) {
			if (!$this->ldapManager
					->bind($currentUser, $currentUser->getPassword())) {
				throw new BadCredentialsException(
						'The credentials were changed from another session.');
			}
		} else {
			if (!$user->getDn()) {
				$userLdap = $this->ldapManager
						->findUserByUsername($user->getUsername());
				if (!$userLdap) {
					throw new BadCredentialsException(
							sprintf('User "%s" not found', $user->getUsername()));
				}
