...
class LdapManagerTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		$this->params = array('baseDn' => 'ou=Groups,dc=example,dc=com',
				'filter' => '(attr0=value0)',
				'attributes' => array(
						array('ldap_attr' => 'uid',
		...
		$this->connection = $this
				->getMock('FR3D\LdapBundle\Driver\LdapConnectionInterface');

		$this->userManager = $this
				->getMock('FR3D\LdapBundle\Model\UserManagerInterface');
		$this->userManager->expects($this->any())->method('createUser')
				->will($this->returnValue(new TestUser()));

		$this->ldapManager = new LdapManager($this->connection,
				$this->userManager, $this->params);
	}
	...
	public function testConstruct() {
		$this->params['attributes'][] = array('ldap_attr' => 'mail',
		...
		$reflectionClass = new \ReflectionClass(
				'FR3D\LdapBundle\Ldap\LdapManager');
		$propertyLdapAttributes = $reflectionClass
				->getProperty('ldapAttributes');
		$propertyLdapAttributes->setAccessible(true);

		$propertyLdapUsernameAttr = $reflectionClass
				->getProperty('ldapUsernameAttr');
		$propertyLdapUsernameAttr->setAccessible(true);

		$this
				->assertEquals(array('uid', 'mail'),
						$propertyLdapAttributes->getValue($this->ldapManager));
		$this
				->assertEquals('uid',
						$propertyLdapUsernameAttr->getValue($this->ldapManager));
	}
	...
	public function testFindUserByUsername() {
		...
		$entries = array('count' => 1,
				array('dn' => 'ou=group, dc=host, dc=foo',
						'uid' => array('count' => 1, 0 => $username,),),);

		$this->connection->expects($this->once())->method('search')
	...
	public function testBuildFilter() {
		...
		$criteria = array('attr1' => 'value1', 'attr2' => 'value2',);
		$expected = '(&(attr0=value0)(attr1=value1)(attr2=value2))';

		$this
				->assertEquals($expected,
						$method->invoke($this->ldapManager, $criteria));
	}
	...
	public function testHydrate() {
		...
		$entry = array('dn' => 'ou=group, dc=host, dc=foo', 'count' => 1,
				'uid' => array('count' => 1, 0 => $username,),);
	...
	public function testHydrateArray() {
		$this->params['attributes'] = array(
				array('ldap_attr' => 'roles', 'user_method' => 'setRoles',),);

		$user = new TestUser();
		$roles = array('count' => 3, 0 => 'ROLE1', 1 => 'ROLE2',
				2 => 'ROLE3',);

		$entry = array('dn' => 'ou=group, dc=host, dc=foo', 'roles' => $roles,);
	...
	public function testBind() {
		...
		$this->connection->expects($this->once())->method('bind')
	...
	public function testEscapeValues() {
		...
		$this
				->assertEquals($expected,
						LdapManager::escapeValue(array($filterval)));
		$this
				->assertEquals(array($expected, $expected, $expected),
						LdapManager::escapeValue(
								array($filterval, $filterval, $filterval)));
	}
	...
	public function testUnescapeValues() {
		...
		$this
				->assertEquals($expected,
						LdapManager::unescapeValue(array($filterval)));
		$this
				->assertEquals(array($expected, $expected, $expected),
						LdapManager::unescapeValue(
								array($filterval, $filterval, $filterval)));
	}
