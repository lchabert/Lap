...
class LdapAuthenticationProviderTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->ldapManager = $this
				->getMock('FR3D\LdapBundle\Ldap\LdapManagerInterface');
		$hideUserNotFoundExceptions = false;

		$this->ldapAuthenticationProvider = new LdapAuthenticationProvider(
				$userChecker, $providerKey, $this->userProvider,
				$this->ldapManager, $hideUserNotFoundExceptions);
	}
	...
	public function testRetrieveUser() {
		...
		$this->userProvider->expects($this->once())
				->method('loadUserByUsername')->with($this->equalTo($username))
				->will($this->returnValue($user));

		$this
				->assertEquals($user,
						$method
								->invoke($this->ldapAuthenticationProvider,
										$username, $token));
	}
	...
	public function testCheckAuthenticationKnownUser() {
		...
		$this->ldapManager->expects($this->once())->method('bind')
	...
	public function testCheckAuthenticationUnknownUser() {
		...
		$this->ldapManager->expects($this->once())
				->method('findUserByUsername')->with($this->equalTo($username))
				->will($this->returnValue($user));
		$this->ldapManager->expects($this->once())->method('bind')
				->with($this->equalTo($user), $this->equalTo($password))
				->will($this->returnValue(true));
	...
	public function testCheckAuthenticationUnknownUserWithoutDn() {
		...
		$method
				->invoke($this->ldapAuthenticationProvider, $userOriginal,
						$token);
