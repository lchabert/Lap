...
class FR3DLdapExtensionTest extends \PHPUnit_Framework_TestCase {
	...
	public function testLoadMinimalConfiguration() {
		$minRequiredConfig = array(
				'client' => array('host' => 'ldap.hostname.local',),
				'user' => array('baseDn' => 'ou=Groups,dc=example,dc=com',),);
		...
		$this
				->assertEquals($defaultConfig['client'],
						$container->getParameter('fr3d_ldap.client.parameters'));
		$this
				->assertEquals($defaultConfig['user'],
						$container
								->getParameter(
										'fr3d_ldap.ldap_manager.parameters'));

		$this
				->assertEquals('fos_user.user_manager',
						(string) $container
								->getAlias('fr3d_ldap.user_manager',
										'user_manager alias failed'));
		$this
				->assertEquals('fr3d_ldap.ldap_manager.default',
						(string) $container
								->getAlias('fr3d_ldap.ldap_manager',
										'ldap_manager alias failed'));
	}
	...
	private function getDefaultConfig() {
		return array(
				'client' => array('host' => 'ldap.hostname.local',
						'port' => 389, 'version' => 3, 'useSsl' => false,
						'useStartTls' => false,),
				'user' => array('baseDn' => 'ou=Groups,dc=example,dc=com',
						'filter' => '',
						'attributes' => array(
				...
				'service' => array('user_manager' => 'fos_user.user_manager',
